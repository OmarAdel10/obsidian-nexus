# Obsidian Link - System Instructions

This file defines the operating protocols for the Obsidian Link extension. It ensures that any agentic workflow using this extension maintains a persistent "Second Brain" and utilizes the Obsidian CLI effectively.
...
- **Identity**: Read `${settings.vault_path}\Gemini\Identity\Developer Profile.md` to load the user's persona and coding preferences.
- **Roadmap**: Read `${settings.vault_path}\Gemini\Brain\${settings.project_name} Brain.md` and `Architecture.md`.
- **Status**: Check the `Plans/` directory for active implementation plans.

### Session Persistence
- **Summarization**: At the end of every session, you MUST create a daily note in `${settings.vault_path}\Gemini\Journal\` using the format `DD-MM-YYYY [Session Name].md`.
- **Templates**: Follow the structure defined in `${settings.vault_path}\Gemini\Templates\Session Log Template.md`.
...
- **File Targeting**: 
    - `file=<name>`: Resolves like a wikilink (no path needed).
...
- **Links**: Prefer `[[Wikilinks]]` for internal vault connections.
